...
# Python venvs put executables in `Scripts` on Windows and `bin` everywhere
# else. Recipes below interpolate this rather than hardcoding either layout.
...
geo-setup:
    ...
    apps/fastapi/.venv/{{venv_bin}}/python -m pip install -r apps/fastapi/requirements.txt
...
geo-dev:
    ...
    cd apps/fastapi && .venv/{{venv_bin}}/python -m uvicorn main:app --reload --port 8000
...
ingest-setup:
    ...
    pipeline/.venv/{{venv_bin}}/python -m pip install --upgrade pip
    pipeline/.venv/{{venv_bin}}/python -m pip install --editable pipeline
...
ingest-plan:
    pipeline/.venv/{{venv_bin}}/wherehouse-ingest plan

# Download any source snapshots not already cached in pipeline/data/raw.
ingest-download *ARGS:
    pipeline/.venv/{{venv_bin}}/wherehouse-ingest download {{ARGS}}

# Build and validate H3 facts without changing Neon.
ingest-build:
    pipeline/.venv/{{venv_bin}}/wherehouse-ingest build
    pipeline/.venv/{{venv_bin}}/wherehouse-ingest validate

# Validate the built facts and atomically activate them in Neon.
ingest-load:
    pipeline/.venv/{{venv_bin}}/wherehouse-ingest load

# Run the complete manual refresh: download, build, validate, and promote.
ingest *ARGS:
    pipeline/.venv/{{venv_bin}}/wherehouse-ingest run {{ARGS}}
